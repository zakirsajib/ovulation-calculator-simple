	...
	<div class="calculator_table">
		<div class="calendar-area">
			<?php if ( ! empty( $options['oc-dates'] ) ):
				printf(__( '<h2>%s</h2>', 'ovulation-calculator' ), esc_attr( $options['oc-dates'] ) );
			endif;
			if ( ! empty( $options['oc-next-month-results'] ) ):
				printf( __( '<p>%s</p>', 'ovulation-calculator' ), esc_attr( $options['oc-next-month-results'] ) );
			endif;
			...
			<div class="fertile" style="padding-top: 1rem;">
				<img class="expected-ovulation" src="<?php echo plugins_url( '/img/circle2.svg' , __FILE__  ); ?>" alt="Days of expected ovulation">&nbsp;&nbsp;&nbsp;<?php printf( __( '%s', 'ovulation-calculator' ), esc_attr( $options['oc-expected-ovulation'] ) ); ?>
			</div>
			<div class="calculateagain">
				<div class="fertile">
					<img class="fertileTick" src="<?php echo plugins_url( '/img/checkmark.svg' , __FILE__  ); ?>" alt="ovulation fertile">&nbsp;&nbsp;&nbsp;<?php printf( __( '%s', 'ovulation-calculator' ), esc_attr( $options['oc-fertile'] ) );?>
				</div>
				<div class="calculateagainbtn" onclick="window.location='http://<?php echo $_SERVER['HTTP_HOST']; ?><?php echo $_SERVER['REQUEST_URI']; ?>'">
					<span class="icon-calendar3"></span>&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="<?php printf(__('%s', 'ovulation-calculator'), esc_attr( $options['oc-change-date'] ) ); ?>" onclick="window.location='http://<?php echo $_SERVER['HTTP_HOST'];?><?php echo $_SERVER['REQUEST_URI']?>'">
				</div>
			</div>
			<div class="fertile">
				<div class="period-indicator"></div>&nbsp;&nbsp;&nbsp;<?php printf( __('%s', 'ovulation-calculator' ), esc_attr( $options['oc-start-ovulation'] ) ); ?>
			</div>
		</div>
	</div>
	...
	<div class="calculator_table">
		<form method="post" id="ovulationCalculatorForm" autocomplete="off">
			<?php if ( ! empty( $options['calculate-ovulation'] ) ) :
				printf( __( ' <h2> %s </h2> ', 'ovulation-calculator' ), esc_attr( $options['calculate-ovulation'] ) );
			endif;
			if ( ! empty( $options['first-day-last-period'] ) ) :
				printf( __( ' <p> %s </p> ', 'ovulation-calculator' ), esc_attr( $options['first-day-last-period'] ) );
			endif; ?>
			<span class="icon-calendar2"></span>
			<?php if ( ! empty( $options['select-date'] ) ) : ?>
			<input type="text" name="something" placeholder="<?php printf( __( '%s', 'ovulation-calculator' ), esc_attr( $options['select-date'] ) ); ?>..." id="calendarInput" readonly/>
			<?php endif; ?>
			<div id="calendar" class="ll-skin-melon"></div>
			
			<?php if ( ! empty( $options['length-cycle'] ) ) :
				printf( __( ' <p> %s </p> ', 'ovulation-calculator'), esc_attr( $options['length-cycle'] ) );
			endif?>
			<select name="days">
				<?php
				for( $i = 20; $i <=45; $i++ ) {
			...
			<div class="submit-btn">
				<?php if ( !empty( $options['oc-submit'] ) ): ?>
					<input type="submit" name="calculator_ok" id="calculatorOk" value="<?php printf(__('%s', 'ovulation-calculator'), esc_attr( $options['oc-submit'] ) ); ?>">
				<?php endif; ?>
			</div>
		</form>
	</div>
